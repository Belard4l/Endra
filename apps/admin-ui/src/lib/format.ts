...
export const PROVIDER_STATUS_LABELS: Record<string, { label: string; className: string }> = {
  confirmed: { label: "Confirmed", className: "bg-emerald-100 text-emerald-800" },
  completed: { label: "Delivered — in dispute window", className: "bg-sky-100 text-sky-800" },
  disputed: { label: "Problem reported", className: "bg-orange-100 text-orange-800" },
  released: { label: "Released for payout", className: "bg-stone-200 text-stone-700" },
  cancelled_by_couple: { label: "Cancelled by couple", className: "bg-stone-200 text-stone-700" },
  cancelled_by_provider: { label: "Cancelled by you", className: "bg-red-100 text-red-800" },
};
