...
export const loginUser = async (req: Request, res: Response, next: NextFunction) => {
    try {
        ...
        if (!user) return next(new AuthError("Account does not exist"));

        if (!user.password) {
            return next(new AuthError("This account uses Google login"));
        }
...
export const refreshToken = async (
    ...
    try {
        ...
        if (!decoded || !decoded.id || !decoded.role) {
            return new JsonWebTokenError("Forbidden: Invalid refreshtoken!");
        }
...
export const createShop = async (
    ...
    try {
        const { name, bio, address, openHours, website, category, sellerId } = req.body;
        if (
            !name ||
            !bio ||
            !address ||
...
export const saveSellerPayment = async (
    req: Request,
    res: Response,
    next: NextFunction
) => {
    try {
        ...
        if (!sellerId || !type) {
            return next(new ValidationError("Seller ID and payment type are required"));
        }
        ...
        if (type === "momo") {
            const { phone_number, momo_code, name } = req.body;
            if (!phone_number || !momo_code || !name) {
                return next(new ValidationError("MTN Mobile Money details are required"));
            }
            const phoneRegex = /^\+?[0-9]\d{1,14}$/;
            if (!phoneRegex.test(phone_number)) {
                return next(new ValidationError("Invalid phone number format"));
            }
            updates.momoPhoneNumber = phone_number;
            updates.momoCode = momo_code;
            ...
            if (!bank_name || !account_number || !account_name) {
                return next(new ValidationError("Bank account details are required"));
            }
            const accountNumberRegex = /^[0-9]{4,32}$/;
            if (!accountNumberRegex.test(account_number)) {
                return next(new ValidationError("Invalid account number format"));
            }
            updates.bankName = bank_name;
            updates.bankAccountNumber = account_number;
            updates.bankAccountName = account_name;
            updates.momoPhoneNumber = null;
            updates.momoCode = null;
            updates.momoName = null;
        } else {
            return next(new ValidationError("Unsupported payment type"));
        }
