...
const NO_REDIRECT = ["/auth/api/logged-in-user", "/auth/api/refresh-token-user", "/auth/api/login-user"];
...
api.interceptors.response.use(
  ...
  async (error: AxiosError) => {
    ...
    if (error.response?.status === 401 && !original?._retry && !url.includes("refresh-token")) {
      ...
      try {
        refreshing = refreshing || axios.post(`${SERVER}/auth/api/refresh-token-user`, {}, { withCredentials: true });
        await refreshing;
        refreshing = null;
        return api(original);
      } catch {
