...
const NO_REDIRECT = ["/auth/api/logged-in-seller", "/auth/api/refresh-token-seller", "/auth/api/login-seller"];
...
api.interceptors.response.use(
  ...
  async (error: AxiosError) => {
    ...
    if (error.response?.status === 401 && !original?._retry && !url.includes("refresh-token")) {
      ...
      try {
        refreshing = refreshing || axios.post(`${SERVER}/auth/api/refresh-token-seller`, {}, { withCredentials: true });
        await refreshing;
        refreshing = null;
        return api(original);
      } catch {
